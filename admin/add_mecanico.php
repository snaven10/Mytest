			...
			<form action='' method='POST' role='form' class='col-xs-12 col-sm-12 col-md-6 col-lg-6' enctype='multipart/form-data'>
				<legend>mecanico</legend>
					<div class='form-group'>
						<input type='text' name='Cod_mecanico' class='form-control'   value='<?php if(!empty($_GET['id'])){echo $row[1];}else{if(!empty($_POST['Cod_mecanico'])){echo $_POST['Cod_mecanico'];}}?>' placeholder='Cod_mecanico' required><br>
...
include '../pie/pie.php';
				if (isset($_POST['add'])) {
					if (!empty($_GET['id'])) {
						if (!empty($_POST['Cod_mecanico']) &&!empty($_POST['Nombre']) &&!empty($_POST['Nit']) &&!empty($_POST['Direccion']) &&!empty($_POST['Telefono'])) {
						if ($mecanico->edit_mecanico($_GET['id'],$_POST['Cod_mecanico'],$_POST['Nombre'],$_POST['Nit'],$_POST['Direccion'],$_POST['Telefono'],1) == 0) { ?>
					...
					if (!empty($_POST['Cod_mecanico']) &&!empty($_POST['Nombre']) &&!empty($_POST['Nit']) &&!empty($_POST['Direccion']) &&!empty($_POST['Telefono'])) {
						include '../clases/clase_mecanico.php';
						$mecanico = new mecanico();
						if ($mecanico->add_mecanico($_POST['Cod_mecanico'],$_POST['Nombre'],$_POST['Nit'],$_POST['Direccion'],$_POST['Telefono'],1) == 0) { ?>

							<script>alert('Se agrego con exito');location.href='view_mecanico.php';</script>
